...
class Input {
    ...
    staticSelect({label, placeholder, blockId, actionId, optional, useGroup=false, initialOptions, options, multi=false}) {
        ...
        const iniOpt = this.#getInitialOption(_options, initialOptions, useGroup, multi);
     ...
     * @return {{optional: boolean, label: {emoji: boolean, text, type: string}, type: string, element}}
     */
    externalSelect({ label, placeholder, blockId, actionId, optional=false, multi=false, minimumQueryLength, maxSelectedItems, initialOptions}) {
        const element = {
